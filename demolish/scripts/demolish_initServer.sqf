...
execVM "scripts/initUtilities.sqf";


if (!isNil "refLogic") then {
	waitUntil { !isNil "refLogic_DONE" };
};

sv_objectiveList = "tower" call sxf_fnc_getEntitiesByPrefix;
...
[ "itemAdd", [ 
	...
	1, "seconds", { opfor countSide allUnits <= 0 || { {!alive _x} count sv_objectiveList isEqualTo 0 } }
] ] call BIS_fnc_loop; 
[ "itemAdd", [ 
	"checkMissionFailed", {
